...
public class Program extends BaseAuditEntity {
    ...
    private void normalizeVietnameseText() {
        programCode = VietnameseTextFixer.fix(programCode);
        programName = VietnameseTextFixer.fix(programName);
        majorName = VietnameseTextFixer.fix(majorName);
        educationLevel = VietnameseTextFixer.fix(educationLevel);
    }
}
